...
def plot():
    ...
    ax.plot(np.arange(len(y5)), y5, marker='o', markersize=20, label='M2 4+4 core (MacBook Air, 2022, Py 3.10)')
    ax.plot(np.arange(len(y6)), y6, marker='o', markersize=20, label='i7-13700K (Custom Desktop, 2023, WSL 2, Py 3.10)')
    ax.plot(np.arange(len(y7)), y7, marker='o', markersize=20, label='M3 Pro 5+6 core (MacBook Pro, 2023, Py 3.12)')
    fig.legend(ncol=2, fontsize=29, bbox_to_anchor=(1, 0.99))
    ax.grid()
